...
public class b2BusinessChange extends JPanel {
	...
	private void registListener(final b2topmanagerui b2ui, final topmanagerJpanel tjpl,
			final b2BusinessChange b2BusinessChange, final String ID) throws RemoteException {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (state == ItemEvent.SELECTED) {
					if (!kuaiDiYuanb.getSelectedItem().equals("增加快递员")) {
						AskDialog ask = new AskDialog(b2ui, "删除快递员", true, "确定删除此快递员？");
						// Object[] options = { "确定", "取消" };
						// int result = JOptionPane.showOptionDialog(null,
						// "删除快递员", "删除该快递员", JOptionPane.YES_NO_OPTION,
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (state == ItemEvent.SELECTED) {
					if (!yeWuYuanb.getSelectedItem().equals("增加业务员")) {
						...
						// Object[] options = { "确定", "取消" };
						// int result = JOptionPane.showOptionDialog(null,
						// "删除业务员", "删除改业务员", JOptionPane.YES_NO_OPTION,
						// JOptionPane.QUESTION_MESSAGE, null, options,
						// options[1]);
						if (ask.state) {
