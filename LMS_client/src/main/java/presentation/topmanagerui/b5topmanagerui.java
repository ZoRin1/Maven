...
public class b5topmanagerui extends JFrame{
	...
	private void registListener(final b5topmanagerui b5topmanagerui) {
		...
		b5topmanagerui.addMouseListener(new MouseAdapter() {
		...
		b5topmanagerui.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent e) {
				// 当鼠标拖动时获取窗口当前位置
				Point p = b5topmanagerui.getLocation();
				// 设置窗口的位置
				// 窗口当前的位置 + 鼠标当前在窗口的位置 - 鼠标按下的时候在窗口的位置
				b5topmanagerui.setLocation(p.x + e.getX() - origin.x, p.y + e.getY()- origin.y);
			}
		});
