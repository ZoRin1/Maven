...
public class b1icwarehousemanui extends JFrame{
	...
	private void registListener(final b1icwarehousemanui b1icwarehousemanui,final icwarehousemanui icwarehousemanui) {
		...
		b1icwarehousemanui.addMouseListener(new MouseAdapter() {
		...
		b1icwarehousemanui.addMouseMotionListener(new MouseMotionAdapter() {
			public void mouseDragged(MouseEvent e) {
				// 当鼠标拖动时获取窗口当前位置
				Point p = b1icwarehousemanui.getLocation();
				// 设置窗口的位置
				// 窗口当前的位置 + 鼠标当前在窗口的位置 - 鼠标按下的时候在窗口的位置
				b1icwarehousemanui.setLocation(p.x + e.getX() - origin.x, p.y + e.getY()- origin.y);
			}
		});
